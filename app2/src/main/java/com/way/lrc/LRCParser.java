...
public class LRCParser {
    ...
    public static void isValidLRCFile(File path) {
        /* SPRD: fix bug 274949
           Coverity CID 70719 Reference is checked against null but then dereferenced anyway @{ */
        if (path == null) {
            throw new IllegalArgumentException("lrc file path is null");
        } else {
            if (!path.exists()) {
                Log.i("liweiping", "lrc is not exists");
                throw new IllegalArgumentException("lrc file [" + path.getPath() + "] is invalid.");
            }
            if (!path.canRead()) {
                Log.i("liweiping", "lrc can not read");
            ...
            if (!path.getName().toLowerCase().endsWith(LyricConstants.EXTENSION)) {
                Log.i("liweiping", "lrc is not a valid lrc file");
    ...
    public static File toUtf(String type, File sourceFile, String tmpFileDir) throws IOException {
        if (LyricConstants.TEXT_UTF_ENCODING.equals(type)) {
            ...
            Writer write = null;
            try {
                File dir = new File(/*StringConstant.CURRENT_PATH*/ tmpFileDir, StringConstant.LRC_DIRECTORY
                        + File.separator);
                if (!dir.exists()) {
                    dir.mkdirs();
                }
